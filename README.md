...
识别到厂商动态刷新参数时，档位卡片显示“自动变频 / idle”及不建议修改或作为新增模板的提示。选中后，编辑/新增区域说明黑屏、切换异常及卡开机风险，并提示选择同面板的 `normal` 普通档位；当前版本不提供此类节点的直接超频操作。

## 1.1.1 镜像兼容性修复

- 保留原分区镜像长度以及文件内部的 AVB footer 位置；更新未签名 AVB 的 DTBO 长度、vbmeta 偏移和 SHA-256/SHA-512 摘要。需要签名密钥的镜像或无法识别的尾部布局会明确阻止打包，避免静默丢弃校验数据。
- 对带 `qcom,mdss-mdp-transfer-time-us` 的命令模式，同步按刷新率反比缩放传输时间，并检查其小于新帧周期。此时“平衡时序”保持原前后肩，仅缩放时钟与传输预算。
- 自动变频/idle 档位含厂商 DDIC 配置和专用命令，不再允许直接克隆成普通高刷档位；请选择同一面板的 `normal` 普通模式。新增节点追加到同级列表末尾，不改变原档位顺序。
- 打包后验证完整镜像的 AVB 摘要、每个 DTB 的字节，以及全部时序节点的实际参数。

本次 `o1_42_02_0a_dsc_cmd` 样本的 144 Hz 配置：从**原镜像**选择 `timing@wqhd_normal_120hz_index_01`，使用“新增独立档位”及“平衡时序”，目标设为 144 Hz。结果应为时钟 `1632000000`、VFP/VBP `16/24`、MDP 传输时间 `6083 µs`。这不是对其他面板的通用参数推荐。

本地实镜像回归测试（镜像与主机 DTC 不随仓库分发）：

```powershell
.\gradlew.bat testDebugUnitTest assembleDebug "-PsampleDir=E:/path/to/cs" "-PhostDtc=E:/path/to/dtc.exe"
```

样本目录中放置 `dtbo_b.img` 和已验证可启动的 `dtbo_b_144hz_scaled.img`。测试对比全部设备树属性，并验证用正常 DTB 重建时整个镜像与参考文件逐字节一致。自动测试不代替实机启动验证。
